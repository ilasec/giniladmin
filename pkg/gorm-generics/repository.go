...
func (r *GormRepository[M, E]) Insert(ctx context.Context, entity *E) error {
	...
	err := r.db.Debug().WithContext(ctx).Create(&model).Error
	if err != nil {
		return err
	}
...
func (r *GormRepository[M, E]) Delete(ctx context.Context, entity *E) error {
	var start M
	model := start.FromEntity(*entity).(M)
	err := r.db.Debug().WithContext(ctx).Delete(model).Error
	if err != nil {
		return err
	}
	return nil
...
func (r *GormRepository[M, E]) DeleteById(ctx context.Context, id any) error {
	var start M
	err := r.db.Debug().WithContext(ctx).Delete(&start, &id).Error
	if err != nil {
		return err
	}
...
func (r *GormRepository[M, E]) Update(ctx context.Context, entity *E) error {
	...
	err := r.db.Debug().WithContext(ctx).Save(&model).Error
	if err != nil {
		return err
	}
...
func (r *GormRepository[M, E]) FindByID(ctx context.Context, id any) (E, error) {
	var model M
	err := r.db.Debug().WithContext(ctx).First(&model, id).Error
	if err != nil {
		return *new(E), err
	}
...
func (r *GormRepository[M, E]) First(ctx context.Context, specifications ...Specification) ([]E, error) {
	return r.FindWithLimit(ctx, 1, -1, specifications...)
}
...
func (r *GormRepository[M, E]) getPreWarmDbForSelect(ctx context.Context, specification ...Specification) *gorm.DB {
	dbPrewarm := r.db.Debug().WithContext(ctx)
